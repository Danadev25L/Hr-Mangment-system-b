...
import { Card, Table, Tag, Space, message, Row, Col, Statistic, Alert } from 'antd';
...
export default function EmployeeHolidaysPage() {
  ...
  const fetchHolidays = async () => {
    setLoading(true);
    try {
      const response = await api.get('/api/employee/holidays');
      ...
      setStats({ totalHolidays: total, upcomingHolidays: upcoming.length });
    ...
    <div className="p-6">
      {nextHoliday && (
        <Alert
          message={
            <span>
              <strong>Next Holiday:</strong> {nextHoliday.name} on {dayjs(nextHoliday.date).format('MMMM DD, YYYY')}
            </span>
          }
          type="info"
